...
<style>
    ...
    .badge-honmei { background: #dc2626; color: white; padding: 3px 10px; border-radius: 16px; font-weight: bold; font-size: 0.82rem; }
    .badge-taikou { background: #059669; color: white; padding: 3px 10px; border-radius: 16px; font-weight: bold; font-size: 0.82rem; }
    .badge-tanana { background: #2563eb; color: white; padding: 3px 10px; border-radius: 16px; font-weight: bold; font-size: 0.82rem; }
...
# ---------------------------------------------------------
# Parsing Utility & Odds Formatting
# ---------------------------------------------------------
...
def parse_db_netkeiba(soup):
    ...
    for r in rows:
        ...
        data_list.append({
            ...
            '騎手': jockey_name,
            '斤量': weight_val, '単勝オッズ': format_odds_val(odds_val), '人気': pop_val,
            '脚質': leg_style,
...
def parse_race_netkeiba(soup):
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        data_list.append({
            '枠番': wakaban, '馬番': umaban, '馬名': horse_name,
            '騎手': jockey_name,
            '斤量': weight_val, '単勝オッズ': format_odds_val(odds_val),
            '人気': pop_val,
            '脚質': leg_style,
            '馬体重': hw_str, '体重増減': hw_diff
        })
...
if target_race_id:
    ...
    with st.spinner("🤖 AI多角分析エンジン実行中（過去脚質・血統・騎手・バイアス統合中）..."):
        ...
        if error:
            ...
            with m4:
                h_odds_val = format_odds_val(honmei['単勝オッズ']) if honmei else "ー"
                h_odds_disp = f"{h_odds_val} 倍 ({honmei['人気']}人気)" if honmei and honmei.get('人気') != '未確定' else f"{h_odds_val} 倍"
            ...
            for idx, (horse, col_c) in enumerate(zip(top_3, [c_h1, c_h2, c_h3])):
                with col_c:
                    o_val_str = format_odds_val(horse['単勝オッズ'])
                    o_txt = f"{o_val_str}倍" if o_val_str != '未確定' else "未確定"
                    ...
                    <div class="horse-card {card_styles[idx]}">
                        ...
                        <div style="font-size: 1.35rem; font-weight: 800; color: #0f172a; margin: 8px 0 4px 0; word-break: break-word; white-space: normal;">
                            {horse['馬番']}番 {horse['馬名']}
                        </div>
                        <p style="font-size: 1.05rem; color: #1e3a8a; font-weight: 800; margin-bottom: 4px;">
                            単勝オッズ: <strong>{o_txt}</strong> ({p_txt})
                        </p>
                        <p style="color: #2563eb; font-weight: bold; font-size: 0.95rem; margin-bottom: 4px;">
                            AIスコア: {horse['AI予想スコア']} pt | 騎手: {horse['騎手']}
                        </p>
                        <p style="color: #059669; font-weight: bold; font-size: 0.88rem; margin-bottom: 4px;">
                        ...
                        <p style="margin: 3px 0; color: #047857; font-size: 0.83rem; word-break: break-word; white-space: normal;">
                            <strong>血統適性:</strong> {horse['血統適性']}
                        </p>
                        <p style="margin: 3px 0; color: #1e40af; font-size: 0.83rem; word-break: break-word; white-space: normal;">
                            <strong>バイアス展開:</strong> {horse['バイアス展開']}
                        </p>
                        <hr style="border-color: #cbd5e1; margin: 8px 0;">
                        <p style="font-size: 0.85rem; color: #334155; line-height: 1.45; word-break: break-word; white-space: normal;">
            ...
            # ---------------------------------------------------------
            # 💰 6. 実績 馬券収支メモ ＆ 回収率グラフ・誤入力削除機能
            # ---------------------------------------------------------
            st.markdown("---")
            st.markdown("### 💰 実績 馬券収支メモ ＆ 回収率・損益推移グラフ")
            ...
            with calc_col1:
                ...
                <div class="calc-card">
                    ...
                    <p style="font-size:1.05rem; margin:4px 0;"><strong>投資合計:</strong> {invest_amount:,} 円</p>
                    <p style="font-size:1.05rem; margin:4px 0;"><strong>払戻合計:</strong> {payout_amount:,} 円</p>
                    <p style="font-size:1.2rem; font-weight:bold; color:{'#059669' if profit >= 0 else '#dc2626'}; margin:4px 0;">
                        損益: {'+' if profit > 0 else ''}{profit:,} 円
                    </p>
                    <p style="font-size:1.05rem; font-weight:bold; color:#2563eb; margin:4px 0;">
                        回収率: {recovery_rate}%
                    </p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with calc_col2:
                ...
                if 'balance_history' in st.session_state and st.session_state['balance_history']:
                    ...
                    st.markdown("##### 📜 記録一覧 ＆ 誤入力削除")
                    st.dataframe(hist_df[['日時', 'レースID', '馬券種', '投資額', '払戻額', '収支', '回収率']], use_container_width=True, height=150)

                    # 間違えた記録の個別削除機能
                    del_options = [f"#{idx+1}: {r['日時']} [{r['レースID']}] {r['馬券種']} ({r['投資額']:,}円→{r['払戻額']:,}円)" for idx, r in enumerate(st.session_state['balance_history'])]
                    selected_del = st.selectbox("🗑️ 間違えて入力した記録を選択して削除:", options=["-- 選択してください --"] + del_options)
                    
                    del_col1, del_col2 = st.columns(2)
                    with del_col1:
                        if st.button("❌ 選択した記録を削除") and selected_del != "-- 選択してください --":
                            target_idx = del_options.index(selected_del) - 1
                            if 0 <= target_idx < len(st.session_state['balance_history']):
                                removed_item = st.session_state['balance_history'].pop(target_idx)
                                st.success(f"記録 #{target_idx+1} を削除しました！")
                                st.rerun()
                    with del_col2:
                        if st.button("🗑️ 履歴を全リセット"):
                            st.session_state['balance_history'] = []
                            st.rerun()
                else:
