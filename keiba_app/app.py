# Kuina AI Racing Pro - Master Ultimate V11.0 (JRA Central Racing Spec)
import re
import requests
...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Kuina AI Racing Pro (JRA特化)",
    page_icon="🏇",
    layout="wide",
...
def calculate_ai_scores(data_list, paddock_status_map=None, race_env=None):
    ...
    for idx, item in enumerate(scored_items):
        ...
        item['予想根拠'] = f"{mark} {item['馬番']}番 {item['馬名']} ({o_str} / {item['騎手']})：{item['_p_comment']}"

    return scored_items

def generate_betting_recommendations(data, strategy_mode="⚖️ バランス重視（王道）", selected_tickets=None):
    if not data or len(data) < 3:
        return {}

    if selected_tickets is None:
        selected_tickets = []

    sorted_data = sorted(data, key=lambda x: x.get('AI予想スコア', 0), reverse=True)
    
    h_honmei = sorted_data[0] if len(sorted_data) > 0 else None
    h_taikou = sorted_data[1] if len(sorted_data) > 1 else None
    h_tanana = sorted_data[2] if len(sorted_data) > 2 else None
    h_renka1 = sorted_data[3] if len(sorted_data) > 3 else None
    h_renka2 = sorted_data[4] if len(sorted_data) > 4 else None

    u_h = str(h_honmei['馬番']) if h_honmei else "1"
    u_o = str(h_taikou['馬番']) if h_taikou else "2"
    u_a = str(h_tanana['馬番']) if h_tanana else "3"
    u_r1 = str(h_renka1['馬番']) if h_renka1 else "4"
    u_r2 = str(h_renka2['馬番']) if h_renka2 else "5"

    bets = {}

    if "🔥 高配当" in strategy_mode:
        bets["馬単"] = {
            "方式": "1着固定 ながし",
            "買い目": f"1着: {u_h}\n相手: {u_o}, {u_a}, {u_r1}, {u_r2}",
            "点数": "4点",
            "想定オッズ": 18.5,
            "解説": f"本命 {u_h}番 の頭固定馬単。1着の軸固定で回収率重視。"
        }
        bets["3連単マルチ"] = {
            "方式": "軸1頭 相手4頭マルチ",
            "買い目": f"軸: {u_h}\n相手: {u_o}, {u_a}, {u_r1}, {u_r2}",
            "点数": "36点",
            "想定オッズ": 120.0,
            "解説": f"本命 {u_h}番 が3着以内に来れば万馬券高配当を狙える高効率マルチ。"
        }
        bets["3連複BOX"] = {
            "方式": "上位5頭 BOX",
            "買い目": f"{u_h}, {u_o}, {u_a}, {u_r1}, {u_r2}",
            "点数": "10点",
            "想定オッズ": 45.0,
            "解説": f"上位5頭のBOX買い。波乱の展開にも柔軟に対応可能。"
        }
    elif "🎯 3連単マルチ" in strategy_mode:
        bets["3連単"] = {
            "方式": "フォーメーション",
            "買い目": f"1着: {u_h}\n2着: {u_o}, {u_a}\n3着: {u_o}, {u_a}, {u_r1}, {u_r2}",
            "点数": "6点",
            "想定オッズ": 85.0,
            "解説": f"本命 {u_h}番 1着固定の厳選3連単フォーメーション。"
        }
        bets["3連単マルチ"] = {
            "方式": "軸2頭マルチ",
            "買い目": f"軸: {u_h}, {u_o}\n相手: {u_a}, {u_r1}, {u_r2}",
            "点数": "18点",
            "想定オッズ": 95.0,
            "解説": f"◎{u_h}番・◯{u_o}番のダブル軸で手堅く超高配当を照準。"
        }
        bets["馬単"] = {
            "方式": "マルチ (2頭折り返し)",
            "買い目": f"{u_h} ⇄ {u_o}, {u_a}",
        ...
        bets["馬連"] = {
            ...
            "点数": "3点",
            "想定オッズ": 12.5,
            "解説": f"本命 {u_h}番 から対抗・単穴・相手への王道馬連3点。"
        }
        bets["ワイド"] = {
            "方式": "BOX",
            "買い目": f"{u_h}, {u_o}, {u_a}",
            "点数": "3点",
            "想定オッズ": 6.8,
            "解説": f"上位評価3頭 ({u_h}, {u_o}, {u_a}) のワイドBOXで高的中率をキープ。"
        }
        bets["3連複"] = {
            "方式": "軸1頭 ながし",
            "買い目": f"軸: {u_h}\n相手: {u_o}, {u_a}, {u_r1}, {u_r2}",
            "点数": "6点",
            "想定オッズ": 28.0,
            "解説": f"本命 {u_h}番 軸で相手4頭へ流す3連複6点買い。"
        }

    if selected_tickets:
        filtered_bets = {}
        for b_name, b_info in bets.items():
            if any(st_t in b_name or st_t in b_info['方式'] for st_t in selected_tickets):
                filtered_bets[b_name] = b_info
        if filtered_bets:
...
def calculate_capital_allocation(tickets, total_budget):
    if not tickets or total_budget <= 0:
        return [], 0, 0
    ...
    synthetic_odds = round(1.0 / inv_sum, 2) if inv_sum > 0 else 0.0

    allocations = []
    total_allocated = 0

    for t in valid_tickets:
        raw_alloc = total_budget * (1.0 / t['odds']) / inv_sum
        alloc_100 = max(100, int(round(raw_alloc / 100.0) * 100))
        expected_payout = int(round(alloc_100 * t['odds']))
        ...
        total_allocated += alloc_100

    return allocations, synthetic_odds, total_allocated
        ...
        if error:
            ...
            with strat_col1:
                strategy_mode = st.radio(
                    "🎯 買い目戦略スタイルの選択:",
                    ["⚖️ バランス重視（王道）", "🔥 高配当・妙味狙い（万馬券特化）", "🎯 3连単マルチ＆BOX特化"],
                    index=0,
                    horizontal=True
                )
            ...
            st.caption("馬単・馬連・ワイド・3連複・3連単および単勝・複勝から選択してトリガミなし資金配分を実行します。")
