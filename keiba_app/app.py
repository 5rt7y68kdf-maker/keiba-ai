...
tab1, tab2, tab3 = st.tabs(["📅 日付から本物レース選択", "🔗 出馬表URL / ID直貼り", "📌 サンプル出馬表"])
...
with tab1:
    ...
    with col_d2:
        if 'fetched_races' in st.session_state and st.session_state['fetched_races']:
            ...
            selected_race_label = st.selectbox("選択:", list(race_options.keys()))
            if st.button("🚀 選択したレースの出馬表をAI解析"):
                target_race_id = race_options[selected_race_label]

with tab2:
    user_url_input = st.text_input("出馬表URL / レースID:")
