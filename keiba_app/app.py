...
def fetch_race_list_by_date(dt_str):
    ...
    if not races_dict:
        return [], f"指定された日付 ({clean_date}) のJRA公式出馬表は見つかりませんでした。（開催日か確認してください）"
...
tab1, tab2, tab3 = st.tabs(["📅 日付から本物レース選択（おすすめ）", "🔗 出馬表URL / ID直貼り", "📌 サンプル出馬表"])
