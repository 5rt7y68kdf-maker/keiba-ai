...
NOISE_WORDS = [
    "競馬新聞", "オッズ", "結果", "映像", "払戻", "掲示板", "データ",
    "My馬券", "出馬表", "予想", "速報", "一覧", "TOP", "トップ", "一覧表"
]
...
def format_race_display(r_id, raw_name=""):
    if len(r_id) != 12: return r_id
    v_code = r_id[4:6]
    venue_name = VENUE_CODE_TO_NAME.get(v_code, "JRA")
    r_num = int(r_id[10:12])
    
    clean_name = clean_race_title(raw_name)
    if clean_name and len(clean_name) >= 2:
        return f"📍【{venue_name}】 {r_num}R  {clean_name}"
    else:
...
# ---------------------------------------------------------
# Web Scraping & Data Fetching (全競馬場・全12R一括取得対応)
# ---------------------------------------------------------
def fetch_html(url):
    headers = {
...
def fetch_race_list_by_date(dt_str):
    ...
    for target_url in urls:
        ...
        containers = soup.select('div.RaceList_Data') or \
                     soup.select('dl.RaceList_Data') or \
                     soup.select('div.RaceList_Box') or \
                     soup.select('div.MainList_Box') or [soup]

        for area in containers:
            for a in area.find_all('a'):
                href = a.get('href', '')
                ...
                title = format_race_display(r_id, a.text)
                if r_id not in races_dict or len(title) > len(races_dict[r_id]['name']):
                    races_dict[r_id] = {
                        'id': r_id,
                        'name': title,
                        'venue': VENUE_CODE_TO_NAME[v_code],
                        'r_num': int(r_id[10:12])
                    }
...
st.markdown("##### 📅 JRA中央競馬 レース検索・選択")
c_date1, c_date2 = st.columns([1, 2])

with c_date1:
    selected_date = st.date_input("開催日を選択 (JST日本標準時):", value=today_jst)
    dt_str = selected_date.strftime("%Y%m%d")
    
    if st.button("🔍 JRA全レース一覧を取得"):
        with st.spinner(f"{selected_date.strftime('%Y/%m/%d')} のJRA公式出馬表を取得中..."):
            races, err = fetch_race_list_by_date(dt_str)
            if err:
                st.warning(err)
            st.session_state['fetched_races'] = races
            st.session_state['last_fetched_date'] = dt_str

with c_date2:
    if 'fetched_races' in st.session_state and st.session_state['fetched_races']:
        races_list = st.session_state['fetched_races']
        race_options = {r['name']: r['id'] for r in races_list}
        
        selected_label = st.selectbox(
            f"📍 {st.session_state.get('last_fetched_date', dt_str)} の全開催競馬場・全レースから選択:",
            options=list(race_options.keys())
        )
        
        if st.button("🚀 選択したレースのAI解析を実行"):
            st.session_state['active_target_id'] = race_options[selected_label]
            st.session_state['active_target_label'] = selected_label
            st.rerun()

# ---------------------------------------------------------
# Active Analysis Section
# ---------------------------------------------------------
if 'active_target_id' in st.session_state and st.session_state['active_target_id']:
    target_race_id = st.session_state['active_target_id']
    target_label = st.session_state.get('active_target_label', f"レースID: {target_race_id}")

    st.markdown("---")
    
    st.markdown(f"""
    <div class="race-banner">
        <div class="race-banner-title">🎯 AI分析中: {target_label}</div>
        <div class="race-banner-sub">12桁レースID: <code>{target_race_id}</code></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.spinner("🤖 AI多角分析実行中（血統・脚質・馬場適性統合中）..."):
        ...
        if error:
            ...
            with m1: st.markdown(f'<div class="metric-container"><div class="metric-label">対象レースID</div><div class="metric-value">{target_race_id}</div></div>', unsafe_allow_html=True)
