...
def fetch_race_list_by_date(dt_str):
    ...
    if not races_dict:
        try:
            dt_obj = datetime.datetime.strptime(clean_date, "%Y%m%d").date()
            if dt_obj.weekday() == 5:
                kai_nichi = "0408"
            elif dt_obj.weekday() == 6:
                kai_nichi = "0409"
            else:
                n_val = (dt_obj.day % 8) + 1
                kai_nichi = f"04{n_val:02d}"
        except Exception:
            kai_nichi = "0408"
        ...
        for venue_name, v_code in active_venues:
            for r_num in range(1, 13):
                r_id = f"{year_str}{v_code}{kai_nichi}{r_num:02d}"
                races_dict[r_id] = {
                    'id': r_id,
...
def change_date(new_date):
    ...
    st.session_state['date_picker_val'] = new_date
    st.session_state.pop('active_race_id', None)

if 'date_picker_val' not in st.session_state:
    st.session_state['date_picker_val'] = st.session_state['sel_date']
...
with d_col1:
    if st.button(f"🏇 今週土曜 ({this_saturday.strftime('%m/%d')})", use_container_width=True):
        change_date(this_saturday)
        st.rerun()
with d_col2:
    if st.button(f"🏇 今週日曜 ({this_sunday.strftime('%m/%d')})", use_container_width=True):
        change_date(this_sunday)
        st.rerun()
with d_col3:
    if st.button(f"📅 本日 ({today_jst.strftime('%m/%d')})", use_container_width=True):
        change_date(today_jst)
        st.rerun()
with d_col4:
    picked_d = st.date_input("日付カレンダー", value=st.session_state['date_picker_val'], key='calendar_widget')
    if picked_d != st.session_state['sel_date']:
        change_date(picked_d)
        st.rerun()
...
if err or not races:
    ...
    # Auto set first race if none active or if current active ID does not belong to this venue/date
