...
# ---------------------------------------------------------
# Web Scraping & Data Fetching (全競馬場・全12R完全対応)
# ---------------------------------------------------------
def fetch_html(url):
    headers = {
...
def fetch_race_list_by_date(dt_str):
    ...
    # race.netkeiba.com を最優先で探索（当日・翌日・開催予定の公式トップ）
    urls = [
    ...
    for target_url in urls:
        ...
        # サイドバー（注目レース等のノイズ）を除外し、メインコンテンツエリアのみ探索
        main_box = (
        ...
        for a in main_box.find_all('a'):
            ...
            r_id = m.group(1)
            v_code = r_id[4:6]
            if v_code not in VENUE_CODE_TO_NAME: continue  # 地方競馬除外

            venue_name = VENUE_CODE_TO_NAME[v_code]
            r_num = int(r_id[10:12])

            # 親要素から綺麗なレース名（オッズ・競馬新聞などのボタン文字を除外）を検出
            parent = a.find_parent(['li', 'tr', 'td', 'dl', 'div'])
            race_title = ""
            if parent:
                title_el = (
                    parent.select_one('.ItemTitle') or
                    parent.select_one('.RaceName') or
                    parent.select_one('td.race_name')
                )
                if title_el:
                    race_title = title_el.text.strip()

            if not race_title:
                raw_txt = a.text.strip()
                if raw_txt not in ['競馬新聞', 'オッズ', '結果', '映像', '払戻', '掲示板', 'データ', '出馬表', 'IPAT']:
                    race_title = raw_txt

            # 不要テキストのろ過
            race_title = re.sub(r'[\n\r\t]+', ' ', race_title)
            race_title = re.sub(r'\s+', ' ', race_title)
            for nw in ['競馬新聞', 'オッズ', '結果', '映像', '払戻', '掲示板', 'データ', '出馬表', '一光', 'パドック', 'IPAT', 'My馬券']:
                race_title = race_title.replace(nw, '')
            race_title = re.sub(r'^(📍|【.*?】|\d+R)\s*', '', race_title).strip()

            if race_title and len(race_title) >= 2 and not race_title.isdigit():
                display_name = f"📍【{venue_name}】 {r_num}R  {race_title}"
            else:
                display_name = f"📍【{venue_name}】 {r_num}R"

            if r_id not in races_dict or (len(race_title) >= 2 and len(races_dict[r_id]['name']) < len(display_name)):
                races_dict[r_id] = {
                    'id': r_id,
                    'name': display_name,
                    'venue': venue_name,
    ...
    if not races_dict:
        return [], f"指定された日付 ({clean_date}) のJRA公式出馬表が見つかりませんでした。"

    races = list(races_dict.values())
    races.sort(key=lambda x: x['id'])  # 競馬場順・1R〜12R順に正しくソート
    return races, None
...
def get_race_data(input_id, paddock_status_map=None, race_env=None, leg_style_overrides=None):
    ...
    # 1. race.netkeiba.com 最優先参照 (当日・前日・開催予定)
    race_urls = [
...
# ---------------------------------------------------------
# UI Header (日本標準時 JST 対応)
# ---------------------------------------------------------
st.markdown("""
<div class="hero-container">
    <div class="hero-title">🏇 Kuina AI Racing Pro</div>
    <div class="hero-sub">JRA中央競馬全開催場・全12R完全対応 AIアナリティクス</div>
</div>
""", unsafe_allow_html=True)

# 日本標準時での今日の日付を取得（1日ズレを完全防止）
today_jst = datetime.datetime.now(JST).date()

st.markdown("#### 📅 開催日・レース選択")
col_d1, col_d2 = st.columns(2)

with col_d1:
    selected_date = st.date_input("開催日を選択 (JST日本時間):", value=today_jst)
    dt_str = selected_date.strftime("%Y%m%d")
    
    if st.button("🔍 JRA全レース一覧を取得"):
        with st.spinner("JRA全開催競馬場・全12Rを探索中..."):
            races, err = fetch_race_list_by_date(dt_str)
            if err: st.error(err)
            else:
                st.session_state['fetched_races'] = races
                st.session_state['last_date_str'] = dt_str

with col_d2:
    target_race_id = None
    if 'fetched_races' in st.session_state and st.session_state['fetched_races']:
        races_data = st.session_state['fetched_races']
        race_options = {r['name']: r['id'] for r in races_data}
        
        selected_race_label = st.selectbox("📍 全開催競馬場・1R〜12Rから選択:", list(race_options.keys()))
        if selected_race_label:
            target_race_id = race_options[selected_race_label]
            st.session_state['active_race_id'] = target_race_id
            st.session_state['active_race_label'] = selected_race_label

# セッション状態の維持
if not target_race_id and 'active_race_id' in st.session_state:
...
if target_race_id:
    ...
    active_label = st.session_state.get('active_race_label', f"レースID: {target_race_id}")
    st.markdown(f'<div class="target-race-banner">🎯 AI分析中: {active_label}</div>', unsafe_allow_html=True)
