...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Kuina AI Racing Pro (JRA中央競馬特化)",
    page_icon="🏇",
    layout="wide",
...
def generate_betting_recommendations(data_list, strategy_mode='⚖️ バランス重視（王道）', selected_ticket_types=None):
    ...
    sorted_h = sorted(data_list, key=lambda x: x.get('AI予想スコア', 0), reverse=True)
    top1 = sorted_h[0] if len(sorted_h) > 0 else {}
    top2 = sorted_h[1] if len(sorted_h) > 1 else {}
    top3 = sorted_h[2] if len(sorted_h) > 2 else {}
    top4 = sorted_h[3] if len(sorted_h) > 3 else {}
    h_uma = top1.get('馬番')
    t_uma = top2.get('馬番')
    a_uma = top3.get('馬番')
    x_uma = top4.get('馬番')
    partner_umas = [d.get('馬番') for d in sorted_h[1:6] if d.get('馬番')]
    all_bets = {}
    if 'バランス' in strategy_mode:
        all_bets['馬連 AI本命軸流し'] = {'方式': f'馬連 1頭軸流し (軸: {h_uma}番)', '買い目': f'軸: {h_uma} ─ 相手: {", ".join([str(u) for u in partner_umas])}', '点数': f'{len(partner_umas)} 点', '解説': 'AI最高評価の本命馬から上位評価馬へ流す王道馬券', '想定オッズ': 14.5}
        all_bets['ワイド 軸1頭ながし'] = {'方式': f'ワイド 1頭軸流し (軸: {h_uma}番)', '買い目': f'軸: {h_uma} ─ 相手: {", ".join([str(u) for u in partner_umas[:3]])}', '点数': f'{len(partner_umas[:3])} 点', '解説': '的中率と回収率のバランスが最も優れた高安定買い目', '想定オッズ': 6.2}
        all_bets['馬単 本命・対抗折り返し'] = {'方式': f'馬単 2頭BOX (軸: {h_uma}, {t_uma}番)', '買い目': f'{h_uma} ⇄ {t_uma}', '点数': '2 点', '解説': 'AI評価1位・2位の裏表本線で勝負', '想定オッズ': 18.0}
        all_bets['3連複 1頭軸流し'] = {'方式': f'3連複 1頭軸流し (軸: {h_uma}番)', '買い目': f'軸: {h_uma} ─ 相手: {", ".join([str(u) for u in partner_umas])}', '点数': f'{len(partner_umas)*(len(partner_umas)-1)//2 if len(partner_umas)>=2 else 1} 点', '解説': 'AI評価上位陣への手広カバーで万馬券も狙える勝負馬券', '想定オッズ': 42.0}
    elif '高配当' in strategy_mode:
        ana_target = a_uma if a_uma else h_uma
        all_bets['馬単 穴一発流し'] = {'方式': f'馬単 1頭軸流し (軸: {ana_target}番)', '買い目': f'軸: {ana_target} ─ 相手: {", ".join([str(u) for u in partner_umas if u != ana_target] + [str(h_uma)])}', '点数': f'{len(partner_umas)} 点', '解説': '単穴・特注馬の頭激走による特大万馬券狙い', '想定オッズ': 85.0}
        all_bets['3連複 穴軸流し'] = {'方式': f'3連複 穴馬軸流し (軸: {ana_target}番)', '買い目': f'{ana_target} ＝ {", ".join([str(u) for u in partner_umas if u != ana_target] + [str(h_uma)])}', '点数': f'{len(partner_umas)*(len(partner_umas)-1)//2 if len(partner_umas)>=2 else 1} 点', '解説': '穴馬絡みの波乱決着で万馬券級の特大高配当をカバー', '想定オッズ': 65.0}
        all_bets['3連単 穴頭波乱フォーメーション'] = {'方式': '3連単 穴固定フォーメーション', '買い目': f'1着: {", ".join([str(u) for u in [a_uma, x_uma] if u])}\n2着: {", ".join([str(u) for u in [h_uma, t_uma, a_uma, x_uma] if u])}\n3着: {", ".join([str(u) for u in partner_umas])}', '点数': '12〜24 点', '解説': '人気馬の着崩れ・荒れるレースを想定した高回収率勝負馬券', '想定オッズ': 220.0}
    elif '3連単' in strategy_mode:
        all_bets['馬単 評価上位BOX'] = {'方式': '馬単 BOX (上位3頭)', '買い目': f'BOX: {", ".join([str(u) for u in [h_uma, t_uma, a_uma] if u])}', '点数': '6 点', '解説': 'AI評価上位3頭による馬単BOXで1着2着の順不同取りこぼしを完全ガード', '想定オッズ': 18.0}
        all_bets['3連単 1頭軸マルチ'] = {'方式': f'3連単 1頭軸マルチ (軸: {h_uma}番)', '買い目': f'軸: {h_uma} ↔ 相手: {", ".join([str(u) for u in partner_umas[:4]])}', '点数': f'{len(partner_umas[:4]) * (len(partner_umas[:4])-1) * 3 if len(partner_umas[:4])>=2 else 6} 点', '解説': '本命馬が2着・3着に敗れても取りこぼさない高回収マルチ', '想定オッズ': 140.0}
        all_bets['3連単 2頭軸マルチ'] = {'方式': f'3連単 2頭軸マルチ (軸: {h_uma}, {t_uma}番)', '買い目': f'軸: {h_uma}, {t_uma} ↔ 相手: {", ".join([str(u) for u in partner_umas if u not in [h_uma, t_uma]])}', '点数': f'{len([u for u in partner_umas if u not in [h_uma, t_uma]]) * 6} 点', '解説': '本命・対抗の2頭が3着以内に入れば相手どれでも的中', '想定オッズ': 95.0}
        all_bets['3連複 評価上位4頭BOX'] = {'方式': '3連複 BOX (4頭)', '買い目': 'BOX: ' + ', '.join([str(u) for u in [h_uma, t_uma, a_uma, x_uma] if u]), '点数': '4 点', '解説': '上位評価4頭のBOXで確実な着漏れ防止', '想定オッズ': 18.0}
...
def get_race_data(input_id, paddock_status_map=None, race_env=None):
    ...
    if not data_list:
        race_urls = [f'https://race.netkeiba.com/race/shutuba.html?race_id={clean_id}', f'https://race.netkeiba.com/race/result.html?race_id={clean_id}']
...
# UI Core Component
# ---------------------------------------------------------
st.markdown('<div class="hero-title">🏇 Kuina AI Racing Pro (JRA中央競馬特化)</div>', unsafe_allow_html=True)

now_jst = datetime.datetime.now(JST)
today_jst = now_jst.date()
weekday = today_jst.weekday()  # 0=Mon, ..., 5=Sat, 6=Sun
...
tab1, tab2, tab3 = st.tabs(["📅 今週・日付で中央競馬(JRA)レース全検索", "⚙️ 競馬場・条件直接指定", "🔢 12桁ID直接入力"])
...
with tab1:
    ...
    st.caption("ボタンひとつで今週の土曜日・日曜日の全レースを一括検索取得できます。")
    ...
    with col_d1:
        ...
        if st.button("🔍 JRA全レース一覧を取得", use_container_width=True) or st.session_state.get('trigger_fetch'):
            ...
            with st.spinner(f"netkeibaから {selected_date.strftime('%Y年%m月%d日')} の中央競馬(JRA)全レースを取得中..."):
    ...
    # Auto-fetch default week races if not in session_state
    ...
    with col_d2:
        if 'fetched_races' in st.session_state and st.session_state['fetched_races']:
            st.success(f"✅ {st.session_state.get('fetched_date_str', '')} 中央競馬(JRA) 全{len(st.session_state['fetched_races'])}レース 取得完了")
            ...
            st.info("上の「今週の土曜日」「今週の日曜日」ボタンを押すと、今週の全JRAレースが一括取得されます。")

with tab2:
    st.markdown("#### ⚙️ 競馬場・レース番号 直接指定")
    c1, c2, c3, c4, c5 = st.columns(5)
    with c1: year_sel = st.number_input("年", 2000, 2026, today_jst.year)
    with c2: venue_sel = st.selectbox("競馬場(JRA)", list(VENUE_MAP.keys()), index=4)
    ...
    if st.button("🚀 条件指定で解析", use_container_width=True):
...
if not target_race_id:
    st.markdown("---")
    st.info("💡 画面上の「今週の土曜日」「今週の日曜日」ボタンを押すと、中央競馬(JRA)の全レース一覧が選べてAI予想が表示されます。")
        ...
        if error:
            ...
            with strat_col2:
                selected_tickets = st.multiselect(
                    ...
                    options=["馬単", "馬連", "ワイド", "3連複", "3連単", "マルチ", "BOX"], default=[]
                )
            ...
            for b_name, b_info in bets.items():
                ...
                sim_option_dict[lbl] = {'name': f"{b_name} ({b_info['方式']})", 'odds': b_info.get('想定オッズ', 10.0)}
            ...
            for d in top_ranked[:1]:
                try:
                    ...
                    sim_option_dict[lbl_t] = {'name': f"単勝 {d['馬番']}番 {d['馬名']}", 'odds': o_val}
                except ValueError: pass
            for d in top_ranked[:2]:
                try:
                    ...
                    sim_option_dict[lbl_f] = {'name': f"複勝 {d['馬番']}番 {d['馬名']}", 'odds': f_odds}
                except ValueError: pass
            ...
            with sim_col1:
                ...
                selected_sim_labels = st.multiselect(
                    ...
                    options=all_option_keys, default=default_selections
                )
            with sim_col2:
            ...
            with calc_col1:
                ...
                if st.button("📝 この収支結果をトータル履歴に記録・追加"):
                    ...
                    st.session_state['balance_history'].append({
                        ...
                        "レースID": target_race_id, "馬券種": bet_type, "投資額": invest_amount,
                        "払戻額": payout_amount, "収支": profit, "回収率": f"{recovery_rate}%"
                    })
            ...
            st.dataframe(
                ...
                use_container_width=True, column_config=column_config, hide_index=True, height=480
            )
            csv = df_display.to_csv(index=False, encoding='utf-8-sig')
            st.download_button(
                label="📥 分析結果（CSV）を保存する", data=csv,
                file_name=f"keiba_ai_{target_race_id}.csv", mime="text/csv"
            )