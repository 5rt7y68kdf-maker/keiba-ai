...
def generate_betting_recommendations(data_list, strategy_mode="⚖️ バランス重視（王道）", selected_ticket_types=None):
    ...
    sorted_data = sorted(data_list, key=lambda x: x.get('AI予想スコア', x.get('_raw_score', 0)), reverse=True)
    if not sorted_data:
        return {}

    honmei = next((d for d in sorted_data if '◎' in d.get('予想印', '')), sorted_data[0])
    taikou = next((d for d in sorted_data if '◯' in d.get('予想印', '')), sorted_data[1] if len(sorted_data) > 1 else None)
    tanana = next((d for d in sorted_data if '▲' in d.get('予想印', '')), sorted_data[2] if len(sorted_data) > 2 else None)
    anama = next((d for d in sorted_data if '☆' in d.get('予想印', '')), sorted_data[5] if len(sorted_data) > 5 else None)

    h_uma = extract_num(honmei['馬番'])
    t_uma = extract_num(taikou['馬番']) if taikou else None
    a_uma = extract_num(tanana['馬番']) if tanana else None
    x_uma = extract_num(anama['馬番']) if anama else None

    all_partner_umas = [extract_num(d['馬番']) for d in sorted_data[1:8] if extract_num(d['馬番']) != h_uma]

    all_bets = {}

    p_4 = all_partner_umas[:4]
    p_6 = all_partner_umas[:6]

    # 1. 馬連
    all_bets["馬連 軸1頭流し(標準)"] = {
        "方式": f"馬連 流し (軸: {h_uma}番)",
        "買い目": f"{h_uma} － " + ", ".join([str(u) for u in p_4]),
        "点数": f"{len(p_4)} 点",
        "解説": "本命◎から上位4頭へ流す高的中率バランス馬券",
        "想定オッズ": 11.5
    }
    all_bets["馬連 軸1頭広め流し(穴カバー)"] = {
        "方式": f"馬連 広め流し (軸: {h_uma}番)",
        "買い目": f"{h_uma} － " + ", ".join([str(u) for u in p_6]),
        "点数": f"{len(p_6)} 点",
        "解説": "相手を6頭まで広げ、中穴・紐荒れをしっかりカバー",
        "想定オッズ": 16.8
    }
    all_bets["馬連 上位5頭BOX"] = {
        "方式": "馬連 5頭BOX",
        "買い目": "BOX: " + ", ".join([str(u) for u in [h_uma] + p_4]),
        "点数": "10 点",
        "解説": "上位評価5頭のBOXで、本命飛び・波乱決着にも完全対応",
        "想定オッズ": 19.5
    }
    ...
    all_bets["馬単 1着固定流し"] = {
        ...
        "買い目": f"1着: {h_uma} ➔ 2着: " + ", ".join([str(u) for u in p_4]),
        "点数": f"{len(p_4)} 点",
        "解説": "本命◎の1着突破に勝負する回収率重視馬券",
        "想定オッズ": 18.2
    }
    all_bets["馬単 1頭軸マルチ(裏表)"] = {
        "方式": f"馬単 1頭軸マルチ (軸: {h_uma}番)",
        "買い目": f"{h_uma} ↔ " + ", ".join([str(u) for u in p_4]),
        "点数": f"{len(p_4) * 2} 点",
        "解説": "本命◎が2着に敗れても取りこぼさないマルチガード",
        "想定オッズ": 24.0
    }

    # 3. ワイド
    all_bets["ワイド 軸流し"] = {
        "方式": f"ワイド 1頭軸流し (軸: {h_uma}番)",
        "買い目": f"{h_uma} － " + ", ".join([str(u) for u in p_4]),
        "点数": f"{len(p_4)} 点",
        "解説": "的中率抜群。手堅くプラス収支を積み重ねる安定馬券",
        "想定オッズ": 4.2
    }
    ...
    all_bets["3連複 1頭軸流し"] = {
        ...
        "買い目": f"{h_uma} ＝ " + ", ".join([str(u) for u in p_6]),
        "点数": f"{len(p_6)*(len(p_6)-1)//2 if len(p_6)>=2 else 1} 点",
        "解説": "軸馬固定で相手6頭に流し、好配当を狙い撃つ王道3連複",
        "想定オッズ": 32.5
    }
    if t_uma:
        p_6_not_t = [u for u in p_6 if u != t_uma]
        all_bets["3連複 2頭軸流し"] = {
            "方式": f"3連複 2頭軸流し (軸: {h_uma}, {t_uma}番)",
            "買い目": f"{h_uma}, {t_uma} ＝ " + ", ".join([str(u) for u in p_6_not_t]),
            "点数": f"{len(p_6_not_t)} 点",
            "解説": "本命◎と対抗◯のダブル軸で点数を絞り、高資金投入で勝負",
            "想定オッズ": 22.0
        }
    all_bets["3連複 上位5頭BOX"] = {
        "方式": "3連複 5頭BOX",
        "買い目": "BOX: " + ", ".join([str(u) for u in [h_uma] + p_4]),
        "点数": "10 点",
        "解説": "上位5頭で全組み合わせをカバー。3着までの混戦を全網羅",
        "想定オッズ": 28.0
    }

    # 5. 3連単
    second_candidates = [u for u in [t_uma, a_uma, x_uma] if u]
    if not second_candidates:
        second_candidates = p_4[:2]
    all_bets["3連単 1着固定フォーメーション"] = {
        "方式": "3連単 1着固定フォーメーション",
        "買い目": f"1着: {h_uma}\n2着: {', '.join([str(u) for u in second_candidates])}\n3着: {', '.join([str(u) for u in p_6])}",
        "点数": f"{len(second_candidates) * max(1, len(p_6) - 1)} 点",
        "解説": "本命◎1着固定で相手を多角展開。特大万馬券の直撃を狙う",
        "想定オッズ": 98.0
    }
    all_bets["3連単 1頭軸マルチ"] = {
        "方式": f"3連単 1頭軸マルチ (軸: {h_uma}番)",
        "買い目": f"軸: {h_uma} ↔ 相手: {', '.join([str(u) for u in p_4])}",
        "点数": f"{len(p_4) * (len(p_4)-1) * 3 if len(p_4)>=2 else 6} 点",
        "解説": "本命◎が2着・3着になっても的中する全着順マルチ展開",
        "想定オッズ": 145.0
    }
    if t_uma:
        p_6_not_t = [u for u in p_6 if u != t_uma]
        all_bets["3連単 2頭軸マルチ"] = {
            ...
            "買い目": f"軸: {h_uma}, {t_uma} ↔ 相手: {', '.join([str(u) for u in p_6_not_t])}",
            "点数": f"{len(p_6_not_t) * 6} 点",
            "解説": "本命・対抗の2頭が3着以内に入れば高配当が確定するマルチ勝負",
            "想定オッズ": 110.0
        }

    # 6. 単勝 & 複勝
    h_odds = float(honmei['単勝オッズ']) if honmei.get('単勝オッズ') != "未確定" else 3.5
    all_bets["単勝 本命直撃"] = {
        "方式": f"単勝 ({h_uma}番 {honmei['馬名']})",
        "買い目": f"単勝: {h_uma}番",
        "点数": "1 点",
        "解説": "シンプルかつ最強。本命◎の単勝回収を狙うダイレクト勝負",
        "想定オッズ": h_odds
    }
    if taikou and taikou.get('単勝オッズ') != "未確定":
        t_odds = float(taikou['単勝オッズ'])
        f_odds = round(max(1.1, t_odds * 0.35), 1)
        all_bets["複勝 対抗押さえ"] = {
            "方式": f"複勝 ({t_uma}番 {taikou['馬名']})",
            "買い目": f"複勝: {t_uma}番",
            "点数": "1 点",
            "解説": "対抗◯の複勝権内(3着以内)で手堅く資金回収",
            "想定オッズ": f_odds
        }
    ...
    if selected_ticket_types and len(selected_ticket_types) > 0:
        ...
        for k, v in all_bets.items():
            for t_type in selected_ticket_types:
                if t_type.lower() in k.lower() or t_type.lower() in v['方式'].lower():
                    filtered[k] = v
                    break
        if filtered:
            return filtered
...
def calculate_capital_allocation(tickets, total_budget):
    ...
    for t in tickets:
        ...
        try:
            ...
            if o_val > 0:
                t_copy = dict(t)
                t_copy['odds_num'] = o_val
                valid_tickets.append(t_copy)
    ...
    inv_sum = sum(1.0 / t['odds_num'] for t in valid_tickets)
    synthetic_odds = round(1.0 / inv_sum, 2) if inv_sum > 0 else 0

    results = []
    for t in valid_tickets:
        raw_alloc = total_budget / (t['odds_num'] * inv_sum)
        alloc_100 = max(100, round(raw_alloc / 100.0) * 100)
        expected_payout = round(alloc_100 * t['odds_num'])
    ...
    with st.spinner("🤖 AI多角分析エンジン実行中（血統・騎手・バイアス・展開統合中）..."):
        ...
        if error:
            ...
            with sim_col1:
                ...
                default_selections = all_option_keys if select_all_tickets else all_option_keys[:6]
