    ...
    for target_url in urls:
        ...
        for a in soup.find_all('a'):
            ...
            if m:
                ...
                # 中央競馬 (JRA: 会場コード 01〜10) のみに限定フィルター
                if v_code not in VENUE_CODE_TO_NAME:
                    continue

                txt = a.text.strip().replace('\n', ' ')
                txt = re.sub(r'\s+', ' ', txt)
                if not txt or len(txt) < 2 or any(kw in txt for kw in ['出馬表', 'オッズ', '結果', '映像', '払戻', 'データベース']):
                    continue

                venue_name = VENUE_CODE_TO_NAME[v_code]
                clean_txt = re.sub(r'^📍\s*', '', txt)
                clean_txt = re.sub(r'【.*?】', '', clean_txt).strip()
                display_title = f"📍【{venue_name}】 {clean_txt}"

                if not any(r['id'] == r_id for r in races):
                    races.append({'id': r_id, 'name': display_title, 'venue': venue_name})
...
# ---------------------------------------------------------
# AI Score Engine (5 Factor Analysis)
# ---------------------------------------------------------
...
<div class="hero-container">
    <div class="hero-title">🏇 Kuina AI Racing Pro (JRA中央競馬専用)</div>
    <div class="hero-sub">ADVANCED QUANT & MULTI-STRATEGY RACING ANALYTICS</div>
</div>
""", unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(["📅 日付でJRA全レース検索", "⚙️ 競馬場・条件指定", "🔢 12桁ID直接入力"])
...
with tab1:
    ...
    with col_d2:
        if 'fetched_races' in st.session_state and st.session_state['fetched_races']:
            ...
            selected_race_label = st.selectbox("開催競馬場・レースを選択してください:", list(race_options.keys()))
            if selected_race_label:
                target_race_id = race_options[selected_race_label]

with tab2:
    st.markdown("#### ⚙️ 開催条件からレースを指定")
    c1, c2, c3, c4, c5 = st.columns(5)
    with c1: year_sel = st.selectbox("年", list(range(2026, 2019, -1)), index=0)
    with c2: venue_sel = st.selectbox("競馬場", list(VENUE_MAP.keys()), index=4) # JRA 10場
    with c3: kai_sel = st.selectbox("回", [f"{i}回" for i in range(1, 13)], index=1)
    with c4: nichi_sel = st.selectbox("日目", [f"{i}日目" for i in range(1, 13)], index=1)
    with c5: race_num_sel = st.selectbox("レース", [f"{i}R" for i in range(1, 13)], index=10) # 11Rデフォルト

    kai_int = int(kai_sel.replace("回", ""))
    nichi_int = int(nichi_sel.replace("日目", ""))
    race_int = int(race_num_sel.replace("R", ""))

    generated_id = f"{year_sel}{VENUE_MAP[venue_sel]}{kai_int:02d}{nichi_int:02d}{race_int:02d}"
    
    st.markdown(f"""
    <div style="background:#eff6ff; border:1px solid #bfdbfe; padding:12px 18px; border-radius:10px; margin: 10px 0;">
        <strong style="color:#1e40af;">選択中の条件:</strong> 
        <span style="font-size:1.05rem; font-weight:bold; color:#0f172a;">{year_sel}年 第{kai_int}回 {venue_sel}競馬 {nichi_int}日目 {race_int}R</span>
        <span style="color:#64748b; margin-left:15px; font-family:monospace;">(ID: {generated_id})</span>
    </div>
    """, unsafe_allow_html=True)
    
    if st.button("🚀 この条件でAI解析実行"):
